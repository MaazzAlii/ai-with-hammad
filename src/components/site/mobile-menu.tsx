...
export function MobileMenu({ links }: { links: NavLink[] }) {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={setOpen}>
      <DialogPrimitive.Trigger aria-label="Open menu" className={buttonVariants({ variant: "ghost", size: "icon", className: "lg:hidden" })}>
        <Menu aria-hidden className="size-5" />
      </DialogPrimitive.Trigger>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay className="anim-overlay fixed inset-0 z-50 bg-(--scrim) backdrop-blur-[6px] lg:hidden" />
        <DialogPrimitive.Content className="anim-drop glass-sheet fixed inset-x-3 top-[max(0.75rem,env(safe-area-inset-top))] z-50 flex max-h-[calc(100dvh-1.5rem)] flex-col rounded-[1.75rem] outline-none lg:hidden">
