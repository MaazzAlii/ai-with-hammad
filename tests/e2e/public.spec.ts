...
import { db, newContext, solveCaptcha } from "./helpers";
...
test.describe("public site", () => {
  ...
  test("a wrong captcha answer is rejected and nothing is stored", async ({ browser }) => {
    const ctx = await newContext(browser);
    const page = await ctx.newPage();
    const email = `captcha-${Date.now()}@example.com`;
    await page.goto("/contact");
    await page.getByLabel("Name").fill("Captcha Tester");
    await page.getByLabel("Email").fill(email);
    await page.getByLabel("What would you like to build?").fill("Testing that the security question is enforced on the server.");
    await expect(page.getByTestId("captcha-question")).toHaveText(/What is/);
    await page.locator('input[name="captchaAnswer"]').fill("999");
    await page.waitForTimeout(2600);
    await page.getByRole("button", { name: "Send message" }).click();
    await expect(page.getByText("Please answer the security question correctly.").first()).toBeVisible();
    const sql = db();
  ...
  test("testimonials page and WhatsApp-free fallback render", async ({ page }) => {
    const res = await page.goto("/testimonials");
    expect(res?.status()).toBe(200);
    await expect(page.getByRole("heading", { level: 1 })).toHaveText("Testimonials");
  });
